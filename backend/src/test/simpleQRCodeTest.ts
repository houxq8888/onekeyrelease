...
async function testQRCodeGeneration() {
  ...
  try {
    ...
    const pairingQRCode = await QRCodeService.generatePairingQRCode(
      'test-device-001', 
      'http://localhost:3000',
      { width: 200 }
    );
    ...
    const connectionQRCode = await QRCodeService.generateConnectionQRCode(
      'test-device-002', 
      'http://localhost:3000',
      { width: 250 }
    );
    ...
    // 测试3: URL二维码
    console.log('\n3. 测试URL二维码生成...');
    const urlQRCode = await QRCodeService.generateURLQRCode(
      'https://www.example.com/mobile-app',
      { width: 300 }
    );
    ...
    // 测试4: 文本二维码
    console.log('\n4. 测试文本二维码生成...');
    const textQRCode = await QRCodeService.generateTextQRCode(
      '这是一段测试文本，用于生成二维码',
      { width: 280 }
    );
    ...
    const batchResults = await QRCodeService.batchGenerateQRCode(
      devices,
      async (device) => {

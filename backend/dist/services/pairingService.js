 ...
 * 配对状态
 */
export var PairingStatus;
(function (PairingStatus) {
    PairingStatus["PENDING"] = "pending";
    PairingStatus["SCANNED"] = "scanned";
...
export class PairingService {
    static pairingSessions = new Map();
    static sessionTimeout = 10 * 60 * 1000; // 10分钟超时
    /**
     * 创建新的配对会话
     */
    static async createPairingSession(serverUrl) {
        try {
            // 生成唯一的会话ID和设备ID
     ...
     * 处理二维码扫描
     */
    static async handleQRCodeScan(sessionId, deviceInfo) {
        try {
     ...
     * 完成配对过程
     */
    static async completePairing(sessionId) {
        try {
            const session = this.pairingSessions.get(sessionId);
            if (!session) {
                throw new AppError('配对会话不存在', 404);
     ...
     * 获取配对会话状态
     */
    static getPairingStatus(sessionId) {
        const session = this.pairingSessions.get(sessionId);
        if (!session) {
            return null;
        }
     ...
     * 生成连接二维码
     */
    static async generateConnectionQRCode(deviceId, serverUrl) {
        try {
     ...
     * 处理WebSocket连接
     */
    static async handleWebSocketConnection(deviceId) {
        try {
            // 查找相关的配对会话

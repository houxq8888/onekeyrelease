...
export class TaskService {
     ...
     * 获取用户任务列表
     */
    static async getUserTasks(userId, page = 1, pageSize = 10, sort = '-createdAt', status) {
        try {
            // 检查是否使用内存数据库模式
            if (!isMongoDBConnected()) {
                // 内存数据库模式：获取用户任务
     ...
     * 更新任务进度
     */
    static async updateTaskProgress(taskId, userId, progress, status, step, stepProgress, stepData) {
        try {
            // 检查是否使用内存数据库模式
            if (!isMongoDBConnected()) {
                // 内存数据库模式：更新任务进度
    ...
    static async addTaskLog(taskId, userId, level, message, details) {
        try {
            ...
            await Task.updateOne({ _id: taskId }, { $push: {
                    logs: {
                        timestamp: new Date(),
                        level,
                        message,
     ...
     * 获取任务日志
     */
    static async getTaskLogs(taskId, userId, limit = 100) {
        try {
            // 检查是否使用内存数据库模式
            if (!isMongoDBConnected()) {
                // 内存数据库模式：获取任务日志

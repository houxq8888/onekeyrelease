...
export class MobileService {
     ...
     * 处理内容生成指令
     */
    static async handleGenerateContent(task, params, _platform, deviceId) {
        task.progress = 10;
     ...
     * 处理图片生成指令
     */
    static async handleGenerateImages(task, params, deviceId) {
        task.progress = 50;
        this.checkTaskStatus(task);
        // 发送进度更新
        await WebSocketService.sendProgressUpdate(deviceId, task.taskId, task.progress);
     ...
     * 处理视频生成指令
     */
    static async handleGenerateVideo(task, params, deviceId) {
        task.progress = 50;
        this.checkTaskStatus(task);
        // 发送进度更新
        await WebSocketService.sendProgressUpdate(deviceId, task.taskId, task.progress);
     ...
     * 处理内容发布指令
     */
    static async handlePublishContent(task, params, platform, deviceId) {
        task.progress = 50;
        this.checkTaskStatus(task);
        // 发送进度更新
        await WebSocketService.sendProgressUpdate(deviceId, task.taskId, task.progress);
     ...
     * 处理批量生成指令
     */
    static async handleBatchGenerate(task, params, _platform, deviceId) {

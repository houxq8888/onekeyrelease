import { Template, TemplateFavorite } from '../models/Template';
...
export class TemplateService {
    ...
    // 获取模板列表
    static async getTemplates(userId, filters) {
        const query = {};
        // 添加分类筛选
        if (filters?.category) {
    ...
    // 更新模板
    static async updateTemplate(userId, templateId, templateData) {
        const template = await Template.findById(templateId);
        if (!template) {
            throw new NotFoundError('模板不存在');
        }
    ...
    // 收藏模板
    static async favoriteTemplate(userId, templateId) {
        // 检查模板是否存在
        const template = await Template.findById(templateId);
        if (!template) {
            throw new NotFoundError('模板不存在');
    ...
    // 取消收藏模板
    static async unfavoriteTemplate(userId, templateId) {
        // 检查模板是否存在
        const template = await Template.findById(templateId);
        if (!template) {
            throw new NotFoundError('模板不存在');
    ...
    // 检查模板是否已收藏
    static async isTemplateFavorite(userId, templateId) {
